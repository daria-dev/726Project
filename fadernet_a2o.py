...
fader_optim = optim.Adam(list(fader.encoder.parameters()) + list(fader.dec_layers.parameters())
                            + list(fader.dec_res.parameters()), 
                        lr=float(args.fader_lr), betas=(0.5,0.999))
...
def train_reconst(epoch):
    ...
    for batch, labels in tqdm(train_loader, desc="Epoch {}".format(epoch)):
        ...
        reconst = fader(batch, hot_digits)
        loss = torch.nn.MSELoss()(reconst, batch)
        sum_rec_loss += loss
        loss.backward()
        fader_optim.step()

    print('\nRec Loss: {:.8f}'
          .format(sum_rec_loss/train_size), flush=True)
